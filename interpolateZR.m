...
for cc = whichClusters
    ...
    for bb = 1:whichBlocks
        ...
        [cf, clustCtrX(cc,bb), clustCtrY(cc,bb) ]= getClusterBlockLocalization(subj,movieDate,location,cc,bb);
        assert(isequal(cf,clusterFrames{cc}));

        bestZ(cc,bb) = fZ(clustCtrX, clustCtrY);
        bestR(cc,bb) = fR(clustCtrX, clustCtrY);
