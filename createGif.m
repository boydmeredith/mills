function [im, map]  = createGif(fig, frameNo, map, delayTime, gifName)
...
if frameNo == 1
    ...
    if ~isempty(gifName)
        imwrite(im,map,gifName,'gif','DelayTime',delayTime,'LoopCount',Inf);
    end
else
    im(:,:,1) = rgb2ind(f.cdata,map,'nodither');
    if ~isempty(gifName)
        imwrite(im,map,gifName,'gif','DelayTime',delayTime,'WriteMode','append');
    end
end
